...
def CreateMultiBoxHead(net, data_layer="data", num_classes=[], from_layers=[],
    ...
    for i in range(0, num):
        ...
        if max_size:
            num_priors_per_location = (2 + len(aspect_ratio)) * len(min_size)#每一个位置先验框的数量 3,4,4,4,3,3
        else:
            num_priors_per_location = (1 + len(aspect_ratio)) * len(min_size)
        if flip:
            num_priors_per_location += len(aspect_ratio) * len(min_size) #五个卷积层的特征图上每一个中心点位置产生先验框的数量分别是4，6，6，6，4，4
        step = []
        if len(steps) > i:
            step = steps[i]
        # Create location prediction layer.#创建位置预测层
        name = "{}_mbox_loc{}".format(from_layer, loc_postfix)
        num_loc_output = num_priors_per_location * 4 # 每一个先验框产生4个坐标
        ...
        # Create confidence prediction layer.#创建置信度预测层
        name = "{}_mbox_conf{}".format(from_layer, conf_postfix)
        num_conf_output = num_priors_per_location * num_classes #每一个先验框都要产生num_classes个得分值
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
        ...
        # Create prior generation layer.#创建预先生成层
        ...
        # Create objectness prediction layer.#创建对象预测层
        if use_objectness:
